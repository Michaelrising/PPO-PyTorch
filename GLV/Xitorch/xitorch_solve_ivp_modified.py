...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb 17 19:54:31 2021
...
from _utils.assertfuncs import assert_fcn_params, assert_runtime
from _core.pure_function import get_pure_function, make_sibling
from _impls.integrate.ivp.explicit_rk import rk4_ivp, rk38_ivp
from _impls.integrate.ivp.adaptive_rk import rk23_adaptive, rk45_adaptive
from _utils.misc import set_default_option, TensorNonTensorSeparator, TensorPacker, get_method
#from misc import set_default_option, TensorNonTensorSeparator, TensorPacker, get_method
from _utils.tensor import convert_none_grads_to_zeros
from _docstr.api_docstr import get_methods_docstr
from debug.modes import is_debug_enabled
...
def solve_ivp(fcn: Union[Callable[..., torch.Tensor], Callable[..., Sequence[torch.Tensor]]],
    ...
    **fwd_options
    ...
    # in our method, the output should be a list, so we dont need to test 
    is_y0_list = isinstance(y0, list) or isinstance(y0, tuple)
    dy0dt = fcn(ts[0], y0, *params)
    is_dy0dt_list = isinstance(dy0dt, list) or isinstance(dy0dt, tuple)
    if is_y0_list != is_dy0dt_list:
